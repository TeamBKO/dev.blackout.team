export default {
  methods: {
    async updateMemberStatus(id, user_id, status) {
      const payload = { status, source: this.currentTab };
      let url = `/rosters/${this.rosterInfo.id}/members/status/${id}`;
      ...
      if (!id && this.selected.length) {
        url = `/rosters/${this.rosterInfo.id}/members/status`;
        if (this.selected.length === 1) {
          url = `/rosters/${this.rosterInfo.id}/members/status/${this.selected[0].id}`;
          if (this.applyRoles) {
            Object.assign(payload, { userID: user_id });
          }
        } else {
