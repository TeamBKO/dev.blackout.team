      ...
      <v-card rounded class="mt-n16 mx-auto fill-height" elevation="2">
        <v-tabs v-model="tab" vertical icons-and-text>
          ...
          <v-tab-item eager transition="fade-transition">
            <v-card flat>
              <v-card-title>{{ tabName }}</v-card-title>
              <v-card-text>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn text @click="updatePersonal" :disabled="isDisabled"
                  >Save</v-btn
                >
                  ...
                  <v-list-item class="px-0" two-line>
                    <v-list-item-content>
                      ...
                      <v-list-item-subtitle
                        ><i>{{ email }}</i></v-list-item-subtitle
                      >
                    </v-list-item-content>
                    <v-list-item-action
                      ><update-email-dialog
...
export default {
  ...
  data() {
    return {
      ...
      usernameValid: false,
      passwordDialogResetValid: false,
      ...
      inputFields: {
        ...
        lastName: {
          ...
          validators: [isAlphanumeric('Last name'), minLength('Last name', 3)],
        },
        location: {
          info: {
            label: 'Location',
  ...
  methods: {
    ...
    async makeEditPasswordRequest() {
      const password = this.password.value;

      try {
        const resp = await this.$axios.$post('/password-reset', { password });
        this.$toast.success(
          `We've dispatched an email with further instructions on how to update your email.`,
          { position: 'top-center' }
        );
      } catch (err) {
        console.log(err);
    ...
    assignInitalValuesToFields(item) {
      this.inputFields.firstName.value = item.first_name;
      this.inputFields.lastName.value = item.last_name;
      this.inputFields.location.value = item.location;
      this.inputFields.birthday.value = new Date(item.birthday)
        .toISOString()
        .substr(0, 10);
      this.inputFields.gender.value = item.gender;
      this.inputFields.description.value = item.description;
      this.email = item.email;
    },

    async updateEmail({ code, email }) {
      try {
        const resp = await this.$axios.$patch('/users/update/email', {
          code,
          email,
        });
    ...
    async updatePersonal(data) {
      ...
      try {
        const results = await this.$axios.$patch(`/users/update`, payload);
  ...
  computed: {
    ...
    tabName() {
      return (
        this.selectedTab.charAt(0).toUpperCase() + this.selectedTab.slice(1)
      );
    },
